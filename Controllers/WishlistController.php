...
class WishlistController {
    ...
    public function notifyProductAvailable(array $product) {
        ...
        if (($this->mailConfig['method'] ?? '') !== 'smtp' || empty($smtp['username']) || empty($smtp['password'])) {
            return ['sent' => 0, 'failed' => count($requests), 'total' => count($requests)];
        }
  ...
  <div style="max-width:620px;margin:0 auto;padding:28px 16px;">
    <div style="background:#ffffff;border:1px solid #d7eadf;border-radius:12px;overflow:hidden;">
      ...
      <div style="padding:26px;">
        ...
        <a href="http://localhost/AdminLTE3/Views/front/boutique/product.php?id=' . (int)$product['id'] . '" style="display:inline-block;background:#1A4D3A;color:#ffffff;text-decoration:none;padding:12px 18px;border-radius:999px;font-weight:bold;">Voir le produit</a>
      </div>
    </div>
  </div>
</body>
