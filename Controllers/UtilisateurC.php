...
class UtilisateurC {
    ...
    public function getByEmail(string $email): ?Utilisateur {
        $q = $this->db()->prepare("SELECT * FROM {$this->table} WHERE email=:e");
        $q->execute([':e'=>$email]);
        $r = $q->fetch();
        if (!$r) return null;
        return $this->hydrate($r);
    ...
    public function emailExists(string $email, int $excludeId=0): bool {
        $q = $this->db()->prepare("SELECT COUNT(*) FROM {$this->table} WHERE email=:e AND id!=:id");
        $q->execute([':e'=>$email,':id'=>$excludeId]);
        return (int)$q->fetchColumn() > 0;
    }

    public function insert(Utilisateur $u): void {
        $q = $this->db()->prepare("INSERT INTO {$this->table} (nom,email,password,role,preference_alimentaire,date_inscription,statut_compte,poids,taille,age,sexe) VALUES(:n,:e,:p,'client','standard',NOW(),1,:po,:t,:a,:s)");
    ...
    public function update(Utilisateur $u): void {
        $q = $this->db()->prepare("UPDATE {$this->table} SET nom=:n,email=:e,poids=:po,taille=:t,age=:a,sexe=:s WHERE id=:id");
    ...
    private function hydrate(array $r): Utilisateur {
