...
class ParticipationController
{
    ...
    public function applyProofAction(int $participationId, int $proofId, string $action): array
    {
        ...
        if ($action === 'analyze_ai') {
            return $this->analyzeProofWithAi($participationId, $proofId, $participation, $proof);
        }
        ...
        if ($action === 'apply_ai') {
            ...
            $reason = trim((string)($proof['ai_reason'] ?? ''));
            return [false, $reason !== '' ? $reason : 'Suggestion IA incertaine. Verification manuelle requise.'];
        }

        return [false, 'Action invalide.'];
    }

    private function analyzeProofWithAi(int $participationId, int $proofId, array $participation, array $proof): array
    {
        try {
            $gemini = new DefiGeminiService();
            if (!$gemini->isConfigured()) {
                return [false, 'Cle Gemini non configuree.'];
            }

            $relativePath = str_replace(['\\', '/'], DIRECTORY_SEPARATOR, (string)($proof['file_path'] ?? ''));
            $target = realpath(__DIR__ . '/../' . $relativePath);
            $baseDir = realpath(__DIR__ . '/../storage/uploads/proofs');
            if (!$target || !$baseDir || strpos($target, $baseDir) !== 0 || !is_file($target)) {
                return [false, 'Fichier de preuve introuvable.'];
            }

            $mime = mime_content_type($target) ?: 'application/octet-stream';
            $review = $gemini->reviewProof($participation, $target, $mime);
            $ok = $this->proof->saveAiReview($proofId, $review);
            return [$ok, $ok ? '' : 'Impossible d enregistrer l analyse IA.'];
        } catch (Throwable $e) {
            try {
                $this->proof->saveAiReview($proofId, [
                    ...
                    'reason' => 'Analyse IA indisponible: ' . $e->getMessage(),
                ]);
            } catch (Throwable $ignored) {
            }
            return [false, 'Analyse IA indisponible: ' . $e->getMessage()];
        }
    }
    ...
    public function saveProof(int $participationId, int $userId, array $file): array
    {
        ...
        try {
            $participation = $this->participation->getById($participationId) ?? [];
            $proof = ['file_path' => $relativePath];
            $this->analyzeProofWithAi($participationId, $proofId, $participation, $proof);
        } catch (Throwable $e) {
            try {
                $this->proof->saveAiReview($proofId, [
                    'decision' => 'error',
