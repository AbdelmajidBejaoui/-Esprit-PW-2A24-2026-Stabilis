...
class UserC
{
    ...
    public function listUsers($search = '', $limit = null, $offset = 0, $sort = 'recent')
    {
        ...
        $sortOptions = [
            ...
            'name_desc' => 'nom DESC',
            'email_asc' => 'email ASC',
            'role_asc' => 'role ASC, nom ASC',
            'status_active' => 'statut_compte DESC, nom ASC',
            'status_inactive' => 'statut_compte ASC, nom ASC',
        ];
    ...
    private function buildResetLink(string $token): string
    {
        ...
        $url = sprintf('http://%s/AdminLTE3/Views/front/users/resetPassword.php?token=%s', $host, $token);
        return $url;
    }
    ...
    public function verifyRecaptcha(string $responseToken): bool
    {
        ...
        if ($secret === '' || stripos($secret, 'YOUR_') !== false) {
            return true;
        }

        if (trim($responseToken) === '') {
            error_log('reCAPTCHA verification failed: empty response token.');
            return false;
        }
        ...
        $json = $this->postRecaptchaVerification($payload);
        if ($json === null) {
            error_log('reCAPTCHA verification failed: unable to contact Google siteverify.');
            return false;
        }
        ...
        error_log('reCAPTCHA verification failed: ' . implode(', ', (array)$errors));
        return false;
    }

    private function postRecaptchaVerification(array $payload): ?array
    {
    ...
    public function createEmailVerification(int $userId, string $email)
    {
        ...
        try {
            ...
            $verifyLink = sprintf('http://%s/AdminLTE3/Views/front/users/verifyEmail.php?token=%s', $_SERVER['HTTP_HOST'] ?? 'localhost', $token);
    ...
    public function sendTwoFactorCode(int $userId, string $email)
    {
        ...
        try {
            ...
            $deleteOld = $db->prepare('DELETE FROM two_factor_codes WHERE user_id = :user_id');
            $deleteOld->execute(['user_id' => $userId]);
