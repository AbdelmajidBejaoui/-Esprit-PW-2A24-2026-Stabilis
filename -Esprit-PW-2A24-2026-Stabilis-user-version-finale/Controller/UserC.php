...
class UserC
{
    ...
    private function buildResetLink(string $token): string
    {
        ...
        $basePath = rtrim(str_replace('\\', '/', dirname($_SERVER['SCRIPT_NAME'] ?? '/View/FrontOffice')), '/');
        $url = sprintf('http://%s%s/resetPassword.php?token=%s', $host, $basePath, urlencode($token));
        return $url;
    }
    ...
    public function createEmailVerification(int $userId, string $email)
    {
        ...
        try {
            ...
            $host = $_SERVER['HTTP_HOST'] ?? 'localhost';
            $basePath = rtrim(str_replace('\\', '/', dirname($_SERVER['SCRIPT_NAME'] ?? '/View/FrontOffice')), '/');
            $verifyLink = sprintf('http://%s%s/verifyEmail.php?token=%s', $host, $basePath, urlencode($token));
