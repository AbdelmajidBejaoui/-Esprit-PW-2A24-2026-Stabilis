...
<div class="form-card" style="max-width: 980px; margin:0 auto;">
    <div style="padding:24px; border-bottom:1px solid var(--border-light); display:flex; justify-content:space-between; gap:12px; align-items:center;">
        ...
        <a href="../../front/boutique/pack.php?id=<?php echo (int)$id; ?>" class="btn-secondary"><i class="fas fa-eye"></i> Voir</a>
    </div>
    <div style="padding:28px;">
        ...
        <form method="POST" enctype="multipart/form-data" novalidate>
            ...
            <div class="form-group">
                ...
                <label class="simple-file-label" for="packImage">
                    <input type="file" name="image" id="packImage" class="simple-file-input" accept="image/jpeg,image/png,image/webp">
                    <i class="fas fa-paperclip"></i>
                    <span data-simple-file-label>Choisir une nouvelle photo</span>
                </label>
                <div class="hint">Laissez vide pour garder la photo actuelle.</div>
            </div>
            ...
            <div class="form-group">
                ...
                <div class="pack-edit-products">
                    ...
                    <label class="pack-edit-product" data-stock="<?php echo (int)$produit['stock']; ?>">
                        ...
                        <img src="/AdminLTE3/assets/img/<?php echo htmlspecialchars($produit['image_url'] ?? 'default-product.png'); ?>" onerror="this.src='/AdminLTE3/assets/img/default-product.png'" alt="">
...
.simple-file-input { position:absolute; inline-size:1px; block-size:1px; opacity:0; pointer-events:none; }
.simple-file-label { min-height:52px; width:100%; display:flex; align-items:center; gap:10px; padding:13px 16px; border:1px solid var(--border-light); border-radius:12px; background:#fff; color:var(--text-main); font-weight:800; cursor:pointer; transition:all .18s ease; }
.simple-file-label:hover { border-color:var(--accent-herb); background:var(--accent-herb-light); transform:translateY(-1px); }
.simple-file-label i { color:var(--accent-herb); }
.simple-file-label span { white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
@media (max-width: 850px) { .pack-edit-grid { grid-template-columns:1fr; } }
</style>

<script>
document.addEventListener('DOMContentLoaded', function () {
    const fileInput = document.querySelector('.simple-file-input');
    const fileLabel = document.querySelector('[data-simple-file-label]');

    if (fileInput && fileLabel) {
        fileInput.addEventListener('change', function () {
            fileLabel.textContent = fileInput.files.length ? fileInput.files[0].name : 'Choisir une nouvelle photo';
        });
    }
});
</script>
