...
$allowedSorts = ['recent', 'oldest', 'name_asc', 'name_desc', 'email_asc', 'role_asc', 'status_active', 'status_inactive'];
...
$users = $userC->listUsers($search, $perPage, $offset, $sort);
...
<div class="card card-primary card-outline">
    <div class="card-header">
        <div class="d-flex justify-content-between align-items-center flex-wrap users-list-toolbar">
            <h3 class="card-title mb-2 mb-sm-0">Gestion des comptes</h3>
            <div class="d-flex align-items-center users-list-actions">
                <form method="GET" action="" class="mr-2 users-list-search">
                ...
                <a href="addUser.php" class="btn btn-primary btn-sm users-list-main-btn">
                    <i class="fas fa-user-plus mr-1"></i> Ajouter
                </a>

                <a href="exportUsersPDF.php" class="btn btn-danger btn-sm ml-2 users-list-main-btn" title="Exporter en PDF">
                ...
                <?php if (!empty($users)): ?>
                    <?php foreach ($users as $u): ?>
                        <tr>
                            ...
                            <td class="users-list-row-actions">
                                <a href="updateUser.php?id=<?php echo (int) $u['id']; ?>" class="btn btn-warning btn-xs" title="Modifier">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <a href="blockUser.php?id=<?php echo (int) $u['id']; ?>" class="btn btn-dark btn-xs" title="Bloquer" onclick="return confirm('Bloquer cet utilisateur ?');">
                                    <i class="fas fa-ban"></i>
                                </a>
                                <a href="deleteUser.php?id=<?php echo (int) $u['id']; ?>" class="btn btn-danger btn-xs" title="Supprimer" onclick="return confirm('Supprimer cet utilisateur ?');">
    ...
    <div class="card-footer clearfix">
        <?php
        $searchQuery = ($search !== '' ? '&search=' . urlencode($search) : '') . '&sort=' . urlencode($sort);
        ?>
