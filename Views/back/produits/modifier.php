...
<style>
.simple-file-input { position:absolute; width:1px; height:1px; opacity:0; pointer-events:none; }
.simple-file-label { min-height:46px; display:flex; align-items:center; gap:10px; padding:0 13px; border:1px solid var(--border-light); border-radius:10px; background:#fff; cursor:pointer; color:var(--text-primary); font-weight:800; }
.simple-file-label i { color:var(--accent-herb); }
.simple-file-label span { min-width:0; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; color:var(--text-muted); font-weight:700; }
</style>
        ...
        <form method="POST" id="productForm" enctype="multipart/form-data" novalidate>
            ...
            <div class="form-group">
                ...
                <div style="display:flex; align-items:center; gap:12px;">
                    <img src="/AdminLTE3/assets/img/<?php echo htmlspecialchars($previewImage); ?>" class="product-img" onerror="this.src='/AdminLTE3/assets/img/default-product.png'">
            ...
            <div class="form-group">
                ...
                <label class="simple-file-label" for="image">
                    <input type="file" accept="image/png,image/jpeg,image/webp" name="image" id="image" class="simple-file-input">
                    <i class="fas fa-paperclip"></i>
                    <span data-simple-file-label>Choisir une nouvelle image</span>
                </label>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelectorAll('.simple-file-input').forEach(input => {
        input.addEventListener('change', function () {
            const label = this.closest('.simple-file-label')?.querySelector('[data-simple-file-label]');
            if (label) {
                label.textContent = this.files && this.files.length ? this.files[0].name : 'Choisir une nouvelle image';
            }
        });
    });
