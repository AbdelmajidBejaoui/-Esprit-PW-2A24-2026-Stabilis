<?php 
$title = "Dashboard ecommerce - Stabilis";
...
try {
    $trainingDb = new PDO('mysql:host=localhost;dbname=stabilis;charset=utf8mb4', 'root', '');
    $trainingDb->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $trainingDb->setAttribute(PDO::ATTR_DEFAULT_FETCH_MODE, PDO::FETCH_ASSOC);

    $trainingStats['entrainements'] = (int)$trainingDb->query("SELECT COUNT(*) FROM entrainements")->fetchColumn();
    $trainingStats['catalogue'] = (int)$trainingDb->query("SELECT COUNT(*) FROM entrainements WHERE is_custom = 0")->fetchColumn();
    $trainingStats['personnalises'] = (int)$trainingDb->query("SELECT COUNT(*) FROM entrainements WHERE is_custom = 1")->fetchColumn();
    $trainingStats['seances'] = (int)$trainingDb->query("SELECT COUNT(*) FROM seances_completees")->fetchColumn();
    $trainingStats['calories'] = (float)$trainingDb->query("SELECT COALESCE(SUM(calories),0) FROM seances_completees")->fetchColumn();
    $trainingStats['users_actifs'] = (int)$trainingDb->query("SELECT COUNT(DISTINCT utilisateur_id) FROM seances_completees")->fetchColumn();
    $trainingStats['generations_ia'] = (int)$trainingDb->query("SELECT COUNT(*) FROM generated_sessions")->fetchColumn();
    $trainingStats['kcal_moyenne'] = $trainingStats['seances'] > 0 ? $trainingStats['calories'] / $trainingStats['seances'] : 0;

    $recentTrainingSessions = $trainingDb->query(
...
<style>
        ...
        .training-stat-card:nth-child(2) { border-left-color: #5a8f68; }
        .training-stat-card:nth-child(3) { border-left-color: #C6A15B; }
        .training-stat-card:nth-child(4) { border-left-color: #2C553E; }
    ...
    <div class="dashboard-header">
        <h1 class="dashboard-title">Dashboard ecommerce</h1>
    </div>
    ...
    <section class="training-stats-section">
        <div class="training-stats-header">
            <div>
                <h2>Performance sportive</h2>
            </div>
            <a href="/AdminLTE3/Views/back/entrainements/dashboard.php"><i class="fas fa-chart-line"></i> Voir module</a>
        </div>

        <div class="training-stats-grid">
            <div class="training-stat-card">
                <strong><?php echo (int)$trainingStats['entrainements']; ?></strong>
                <span>Entrainements</span>
            </div>
            <div class="training-stat-card">
                <strong><?php echo (int)$trainingStats['seances']; ?></strong>
                <span>Seances terminees</span>
            </div>
            <div class="training-stat-card">
                <strong><?php echo number_format((float)$trainingStats['calories'], 0, ',', ' '); ?></strong>
                <span>Kcal brulees</span>
            </div>
            <div class="training-stat-card">
                <strong><?php echo (int)$trainingStats['generations_ia']; ?></strong>
                <span>Generations IA</span>
            </div>
        </div>

        <div class="training-stats-grid">
            <div class="training-stat-card">
                <strong><?php echo (int)$trainingStats['catalogue']; ?></strong>
                <span>Catalogue admin</span>
            </div>
            <div class="training-stat-card">
                <strong><?php echo (int)$trainingStats['personnalises']; ?></strong>
                <span>Programmes users</span>
            </div>
            <div class="training-stat-card">
                <strong><?php echo (int)$trainingStats['users_actifs']; ?></strong>
                <span>Users actifs</span>
            </div>
            <div class="training-stat-card">
                <strong><?php echo number_format((float)$trainingStats['kcal_moyenne'], 0, ',', ' '); ?></strong>
                <span>Kcal / seance</span>
            </div>
        </div>

        <table class="training-session-table">
            <thead>
                <tr><th>Utilisateur</th><th>Entrainement</th><th>Duree</th><th>Kcal</th><th>Date</th></tr>
            </thead>
            <tbody>
                <?php foreach ($recentTrainingSessions as $session): ?>
                    <tr>
                ...
                <?php endforeach; ?>
                <?php if (!$recentTrainingSessions): ?>
                    <tr><td colspan="5" style="text-align:center;">Aucune seance sportive pour le moment.</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
    </section>
