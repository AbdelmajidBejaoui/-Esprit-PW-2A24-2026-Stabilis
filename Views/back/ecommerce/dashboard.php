...
$orderStatusBreakdown = $dashboardService->getOrderStatusBreakdown();
$customerStats = $dashboardService->getCustomerStatistics();
$topCustomers = $dashboardService->getTopCustomers();
$avgOrderValue = (float)($customerStats['avg_order_value'] ?? 0);
$totalCustomers = (int)($customerStats['total_customers'] ?? 0);
$ordersWithDiscount = 0;

foreach ($commandes as $commande) {
    if ((int)($commande['discount_percent'] ?? 0) > 0 || (float)($commande['discount_amount'] ?? 0) > 0) {
        $ordersWithDiscount++;
    }
}
?>
...
<style>
        ...
        .commerce-stat-card:nth-child(2) { border-left-color: #5a8f68; }
        .commerce-stat-card:nth-child(3) { border-left-color: #C6A15B; }
        .commerce-stat-card:nth-child(4) { border-left-color: #2C553E; }
        ...
        .commerce-mini-table th,
        .commerce-mini-table td {
            padding: 11px 10px;
    ...
    <section class="commerce-stats-section">
        <div class="commerce-stats-header">
            <div>
                <h2>Resume ecommerce</h2>
            </div>
            <a href="/AdminLTE3/Views/back/commandes/liste.php"><i class="fas fa-shopping-cart"></i> Voir commandes</a>
        </div>

        <div class="commerce-stats-grid">
            <div class="commerce-stat-card">
                <strong><?php echo $totalCustomers; ?></strong>
                <span>Clients uniques</span>
            </div>
            <div class="commerce-stat-card">
                <strong><?php echo number_format($avgOrderValue, 0, ',', ' '); ?> EUR</strong>
                <span>Panier moyen</span>
            </div>
            <div class="commerce-stat-card">
                <strong><?php echo $ordersWithDiscount; ?></strong>
                <span>Commandes promo</span>
            </div>
            <div class="commerce-stat-card">
        ...
        <div class="commerce-table-grid">
            <table class="commerce-mini-table">
                <thead>
                    <tr><th>Statut</th><th>Commandes</th><th>Total</th></tr>
                </thead>
                <tbody>
                    <?php foreach ($orderStatusBreakdown as $statusRow): ?>
                        <tr>
                            <td><?php echo htmlspecialchars($statusRow['statut']); ?></td>
                            <td><?php echo (int)$statusRow['count']; ?></td>
                            <td><strong><?php echo number_format((float)$statusRow['total_revenue'], 2, ',', ' '); ?> EUR</strong></td>
                        </tr>
                    <?php endforeach; ?>
                    <?php if (!$orderStatusBreakdown): ?>
                        <tr><td colspan="3" style="text-align:center;">Aucune commande.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
            ...
            <table class="commerce-mini-table">
                <thead>
                    <tr><th>Client</th><th>Commandes</th><th>Total</th></tr>
                </thead>
                <tbody>
                    <?php foreach ($topCustomers as $customer): ?>
                        <tr>
                            ...
                            <td><strong><?php echo number_format((float)$customer['total_spent'], 2, ',', ' '); ?> EUR</strong></td>
                        </tr>
                    <?php endforeach; ?>
                    <?php if (!$topCustomers): ?>
                        <tr><td colspan="3" style="text-align:center;">Aucun client.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
