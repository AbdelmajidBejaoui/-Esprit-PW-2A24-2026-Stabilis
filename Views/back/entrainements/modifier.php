<?php
$title = "Modifier entrainement - Stabilis";
...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
    ...
    if (!$errors) {
        $entrainement->setNom(trim($_POST['nom']));
        $entrainement->setDescription(trim($_POST['description'] ?? ''));
        $entrainement->setTypeSport($_POST['type_sport']);
        $entrainement->setNiveau($_POST['niveau']);
        $entrainement->setMetValue((float)($_POST['met_value'] ?? 5));
        $entrainement->setIsCustom((int)($_POST['is_custom'] ?? $entrainement->getIsCustom()));
        $controller->update($entrainement);

        $newSteps = [];
        foreach (($_POST['etapes'] ?? []) as $step) {
            if (trim($step['titre'] ?? '') !== '') {
                $newSteps[] = ['titre' => trim($step['titre']), 'description' => trim($step['description'] ?? '')];
            }
        }
        $controller->insertEtapes($id, $newSteps);
...
<div class="training-form-card">
    ...
    <p class="training-form-intro">Mettez a jour les informations visibles dans le catalogue et le programme utilisateur.</p>
    ...
    <form method="POST">
        <div class="training-form-grid">
            <div class="training-field full"><label>Nom de l'entrainement</label><input name="nom" required maxlength="100" placeholder="Ex: Full body debutant" value="<?php echo htmlspecialchars($_POST['nom'] ?? $entrainement->getNom()); ?>"><small>Nom court et clair visible dans le programme.</small></div>
            <div class="training-field full"><label>Description</label><textarea name="description" rows="4" placeholder="Objectif, materiel necessaire, public cible..."><?php echo htmlspecialchars($_POST['description'] ?? $entrainement->getDescription()); ?></textarea></div>
            <div class="training-field"><label>Type de sport</label><select name="type_sport" required>
                    <?php foreach ($sports as $sport): ?><option value="<?php echo htmlspecialchars($sport); ?>" <?php echo ($_POST['type_sport'] ?? $entrainement->getTypeSport()) === $sport ? 'selected' : ''; ?>><?php echo htmlspecialchars($sport); ?></option><?php endforeach; ?>
                </select></div>
            <div class="training-field"><label>Niveau</label><select name="niveau" required>
                    <?php foreach (['debutant'=>'Debutant','intermediaire'=>'Intermediaire','avance'=>'Avance'] as $value => $label): ?><option value="<?php echo $value; ?>" <?php echo ($_POST['niveau'] ?? $entrainement->getNiveau()) === $value ? 'selected' : ''; ?>><?php echo $label; ?></option><?php endforeach; ?>
                </select></div>
            <div class="training-field"><label>Valeur MET</label><input type="number" step="0.1" min="1" max="20" name="met_value" value="<?php echo htmlspecialchars($_POST['met_value'] ?? (string)$entrainement->getMetValue()); ?>" placeholder="Ex: 5.0"><small>Intensite energetique: marche 3, musculation 5-7, HIIT 10+.</small></div>
            <div class="training-field"><label>Source</label><select name="is_custom">
                    <option value="0" <?php echo (int)($_POST['is_custom'] ?? $entrainement->getIsCustom()) === 0 ? 'selected' : ''; ?>>Catalogue admin</option>
                    <option value="1" <?php echo (int)($_POST['is_custom'] ?? $entrainement->getIsCustom()) === 1 ? 'selected' : ''; ?>>Personnalise</option>
                </select><small>Catalogue = modele admin, personnalise = cree pour un utilisateur.</small></div>
            <div class="full">
                <div class="steps-box">
                    <div class="steps-head"><h3>Etapes du tutoriel</h3><p>Gardez des consignes courtes et dans l'ordre de la seance.</p></div>
                    <div id="steps">
                        <?php $shown = $steps ?: [['titre' => '', 'description' => '']]; ?>
                        <?php foreach ($shown as $index => $step): ?>
                            <div class="step-row">
                                <div class="training-field"><label>Titre</label><input name="etapes[<?php echo $index; ?>][titre]" placeholder="Ex: Echauffement" value="<?php echo htmlspecialchars($step['titre'] ?? ''); ?>"></div>
                                <div class="training-field"><label>Consignes</label><textarea name="etapes[<?php echo $index; ?>][description]" rows="2" placeholder="Series, repetitions, posture..."><?php echo htmlspecialchars($step['description'] ?? ''); ?></textarea></div>
...
<script>
let stepIndex = <?php echo count($shown); ?>;
function addStep() {
