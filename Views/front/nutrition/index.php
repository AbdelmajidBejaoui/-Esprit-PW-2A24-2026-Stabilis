...
<head>
    ...
    <style>
        ...
        .nutrition-card { position:relative; overflow:hidden; background:#fff; border:1px solid #e3ece5; border-radius:18px; padding:22px; min-height:190px; box-shadow:0 16px 34px rgba(26,77,58,.08); animation: cardIn .62s cubic-bezier(.22,1,.36,1) both; transition:transform .2s ease, box-shadow .2s ease, border-color .2s ease; }
        .nutrition-card::before { content:""; position:absolute; inset:0 auto 0 0; width:5px; background:#1A4D3A; opacity:.88; }
        .nutrition-card:hover { transform:translateY(-6px); box-shadow:0 24px 44px rgba(26,77,58,.13); border-color:#cfe2d3; }
        .nutrition-card:nth-child(3n+1)::before { background:#1A4D3A; }
        .nutrition-card:nth-child(3n+2)::before { background:#C6A15B; }
        .nutrition-card:nth-child(3n+3)::before { background:#3A6B8F; }
        ...
        .macro-row span { display:inline-flex; align-items:center; border-radius:999px; padding:7px 10px; font-size:12px; font-weight:850; border:1px solid transparent; }
        .macro-row span:nth-child(1) { background:#fff4df; color:#7b5518; border-color:#efd7a7; }
        .macro-row span:nth-child(2) { background:#eaf3fb; color:#24506b; border-color:#cfe2ee; }
        .macro-row span:nth-child(3) { background:#edf6ef; color:#1A4D3A; border-color:#d5e8da; }
        ...
        .tool-card select { width:100%; border:1px solid #dfe8df; border-radius:12px; padding:12px; margin-bottom:12px; font:inherit; }
        .photo-upload-input { position:absolute; width:1px; height:1px; opacity:0; pointer-events:none; }
        .photo-upload-control { display:grid; grid-template-columns:auto 1fr; gap:12px; align-items:center; width:100%; min-height:86px; border:1px dashed #b9d0be; border-radius:16px; padding:14px; margin-bottom:14px; background:#f8fcf9; cursor:pointer; transition:border-color .18s ease, background .18s ease, transform .18s ease; }
        .photo-upload-control:hover { border-color:#1A4D3A; background:#f1faf3; transform:translateY(-1px); }
        .photo-upload-control i { width:46px; height:46px; border-radius:14px; background:#1A4D3A; color:#fff; display:flex; align-items:center; justify-content:center; font-size:20px; }
        .photo-upload-text strong { display:block; color:#1A4D3A; font-size:15px; }
        .photo-upload-text span { display:block; margin-top:4px; color:#6b756d; font-size:13px; line-height:1.35; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
            ...
            <div class="tools-grid">
                ...
                <form class="tool-card" method="POST" enctype="multipart/form-data">
                    ...
                    <label class="photo-upload-control">
                        <input class="photo-upload-input" type="file" name="food_photo" accept="image/jpeg,image/png,image/webp">
                        <i class="fas fa-cloud-arrow-up"></i>
                        <span class="photo-upload-text">
                            <strong>Ajouter une photo</strong>
                            <span data-file-label>Aucun fichier selectionne</span>
                        </span>
                    </label>
    ...
    (() => {
        ...
        document.addEventListener('change', (event) => {
            ...
            if (input instanceof HTMLInputElement && input.type === 'file') {
                const label = input.closest('.photo-upload-control')?.querySelector('[data-file-label]');
                if (label) {
                    label.textContent = input.files?.[0]?.name || 'Aucun fichier selectionne';
                }
            }
