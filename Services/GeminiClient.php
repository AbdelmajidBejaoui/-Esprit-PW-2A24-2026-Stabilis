        ...
        foreach ($models as $index => $model) {
            ...
            for ($attempt = 1; $attempt <= $attempts; $attempt++) {
                try {
                    ...
                    $lastError = $e;
                    if (!$this->isTemporaryGeminiError($e) || $attempt === $attempts) {
                        break;
                    }

                    error_log("Gemini model {$model} temporary failure, retry {$attempt}: " . $e->getMessage());
                    usleep(450000 * $attempt);
                }
            }

            if (!$lastError || !$this->isTemporaryGeminiError($lastError)) {
                throw $temporaryError ?? $lastError ?? new RuntimeException('Gemini generation failed.');
            }

            $temporaryError = $temporaryError ?? $lastError;

            if ($index === count($models) - 1) {
                throw $temporaryError;
            }

            error_log("Gemini model {$model} temporarily unavailable, trying fallback: " . $lastError->getMessage());
            usleep(350000);
        }
